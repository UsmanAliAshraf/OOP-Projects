...
namespace NadraManagementGUI
{
    public partial class frmTrackApplication : frmApplicant
    {
        ...
        {
            citizen person = ObjectHandler.GetCitizenDL().SearchCitizen1(txtCnic.Text);
            if (person != null)
            {
        ...
        private void cmdtrack_Click(object sender, EventArgs e)
        {
            ...
            {
                citizen person = CitizenDBCRUD.citizenFromTokenNumber(int.Parse(txtTrack.Text));
                if (person != null)
                {
                    MessageBox.Show(person.Name +"Dear User,Your application against tacking id  " + person.TokenNumber+ " is in completion mode. We'll get it done soon!");

                }
                else
                {
                    MessageBox.Show("Invalid Tracking ID!");
